...
# Traefik Helm chart omits the "-traefik" suffix when the release name already
# contains "traefik" (standard Helm fullname template behavior).
if [[ "${lbType}" == "TRAEFIK" && "${lbReleasename}" == *"traefik"* ]]; then
  lbFullSvcName="${lbReleasename}"
else
  lbFullSvcName="${lbReleasename}-${lbsvcName}"
fi

if [ ${sslType} == "NONSSL" ]; then
   port=$(kubectl --namespace ${lbNamespace} get services -o jsonpath="{.spec.ports[0].nodePort}" "${lbFullSvcName}")
   protocol="http"
   adminurl="${protocol}://${lbHostname}:${port}"
   soaurl="${protocol}://${lbHostname}:${port}"
   osburl="${protocol}://${lbHostname}:${port}"
elif [ ${sslType} == "SSL" ]; then
   port=$(kubectl --namespace ${lbNamespace} get services -o jsonpath="{.spec.ports[1].nodePort}" "${lbFullSvcName}")
   protocol="https"
   adminurl="${protocol}://${lbHostname}:${port}"
   soaurl="${protocol}://${lbHostname}:${port}"
   osburl="${protocol}://${lbHostname}:${port}"
